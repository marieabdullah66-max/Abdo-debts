...
function exportMovementReportPdf(){
  ...
  const exportedEquivalent=rows.reduce((sum,x)=>sum+Number(x.equivalent_boxes||0),0),exportedUnresolved=rows.filter(x=>!x.item_id).length;
  const rowsHtml=rows.map((x,i)=>{const cat=x.item_catalog||{};const name=x.report_name||cat.item_name||'—';const code=cat.item_code||'';return `<tr><td>${i+1}</td><td class="item-name"><strong>${esc(name)}</strong>${code?`<small>${esc(code)}</small>`:''}</td><td>${movementNumber(x.boxes_sold,2)}</td><td>${movementNumber(x.loose_sold,2)}</td><td>${x.units_per_box?Number(x.units_per_box).toLocaleString('en-US'):'—'}</td><td>${movementNumber(x.equivalent_boxes,2)}</td><td class="rate">${movementNumber(x.daily_rate,3)}</td><td>${x.item_id?'مطابق':'غير مطابق'}</td></tr>`;}).join('');
  popup.document.open();
  popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>تقرير حركة الأصناف - ${esc(branch)}</title><style>@page{size:A4 landscape;margin:9mm}*{box-sizing:border-box}body{font-family:Arial,Tahoma,sans-serif;color:#173b36;margin:0;background:#fff;direction:rtl}.head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #0f6259;padding-bottom:8px;margin-bottom:10px}.head h1{margin:0 0 4px;font-size:20px;color:#0f6259}.meta{font-size:10px;line-height:1.8;color:#536a66}.summary{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin:8px 0 10px}.card{border:1px solid #d7e2df;border-radius:8px;padding:7px;text-align:center}.card span{display:block;font-size:8px;color:#6e817d}.card strong{display:block;margin-top:2px;font-size:12px;color:#173b36}.filters{font-size:9px;background:#f3f8f7;border:1px solid #d7e2df;border-radius:7px;padding:6px 8px;margin-bottom:8px}table{width:100%;border-collapse:collapse;font-size:8px;table-layout:fixed}thead{display:table-header-group}th{background:#0f6259;color:#fff;font-weight:700;padding:5px 4px;border:1px solid #0d574f}td{padding:4px;border:1px solid #dce5e3;text-align:center;vertical-align:middle;word-break:break-word}tbody tr:nth-child(even){background:#f7faf9}.item-name{text-align:right;width:31%}.item-name strong{display:block}.item-name small{display:block;color:#687a76;font-size:7px;margin-top:2px;direction:ltr;text-align:right}.rate{font-weight:700;color:#0f6259}.foot{font-size:8px;color:#75837f;margin-top:7px;display:flex;justify-content:space-between}.no-print{margin:10px 0;text-align:center}@media print{.no-print{display:none}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><div class="head"><div><h1>تقرير حركة الأصناف</h1><div class="meta">الفرع: <strong>${esc(branch)}</strong><br>الفترة: ${esc(r.period_start)} → ${esc(r.period_end)} (${Number(r.days_count||0)} يوم)</div></div><div class="meta">Abdo Debts<br>تاريخ التصدير: ${esc(generated)}</div></div><div class="summary"><div class="card"><span>عدد الأصناف في التصدير</span><strong>${rows.length.toLocaleString('en-US')}</strong></div><div class="card"><span>حركات البيع</span><strong>${Number(r.transaction_count||0).toLocaleString('en-US')}</strong></div><div class="card"><span>إجمالي العلب في التصدير</span><strong>${movementNumber(exportedEquivalent,2)}</strong></div><div class="card"><span>غير المطابق في التصدير</span><strong>${exportedUnresolved.toLocaleString('en-US')}</strong></div></div><div class="filters">الحالة: <strong>${esc(movementStatusText())}</strong> · ترتيب معدل البيع: <strong>${esc(sortText)}</strong>${state.movementSearch?` · البحث: <strong>${esc(state.movementSearch)}</strong>`:''}</div><table><thead><tr><th style="width:4%">#</th><th style="width:31%">الصنف</th><th>علب</th><th>فرط</th><th>فرط/علبة</th><th>إجمالي بالعلب</th><th>معدل/يوم</th><th>المطابقة</th></tr></thead><tbody>${rowsHtml}</tbody></table><div class="foot"><span>معدل البيع اليومي = إجمالي العلب المكافئة ÷ عدد أيام التقرير</span><span>${rows.length.toLocaleString('en-US')} صنف</span></div><div class="no-print"><button id="movementPrintButton" style="font:inherit;padding:8px 18px;border:0;border-radius:8px;background:#0f6259;color:#fff;cursor:pointer">حفظ كـ PDF / طباعة</button></div></body></html>`);
