...
async function supplierDetailView(main){
  ...
  <section class="supplier-hero"><div><div class="muted">ملف المورد</div><h2>${esc(supplier.name)}</h2><div class="supplier-contact">${supplier.phone?`📞 ${esc(supplier.phone)}`:''}${supplier.notes?`<span>${esc(supplier.notes)}</span>`:''}</div></div></section>
...
function invoiceModal(i=null,opts={}){const lockedSupplier=opts.supplierId||'';const activeBranches=state.branches.filter(b=>b.active||b.id===i?.branch_id);showModal(`${i?'تعديل':'إضافة'} فاتورة`,`<form id="invoiceForm" class="form-grid">
 <div class="field full"><label>المورد *</label><div style="display:flex;gap:7px"><select class="select" name="supplier_id" id="invSupplier" required style="flex:1">${supplierOptions()}</select>${can('manage_suppliers')&&!lockedSupplier?'<button type="button" class="btn btn-soft" id="quickSupplier">+ مورد</button>':''}</div></div>
 ...
 const f=document.getElementById('invoiceForm');f.elements.supplier_id.value=i?.supplier_id||lockedSupplier||'';if(lockedSupplier)f.elements.supplier_id.disabled=true;f.elements.branch_id.value=i?.branch_id||'';const quick=document.getElementById('quickSupplier');if(quick)quick.onclick=()=>supplierModal(null,s=>{const sel=document.getElementById('invSupplier');sel.innerHTML=supplierOptions();sel.value=s.id;});}
