...
async function paymentPlansView(main){
  ...
  <section class="panel payment-plan-filter-panel"><div class="toolbar payment-plan-toolbar"><input class="input" id="paymentPlanSearch" placeholder="بحث باسم المورد..." value="${esc(state.paymentPlanSearch||'')}"><select class="select" id="paymentPlanBranch">${branchOptions(true,false)}</select><select class="select" id="paymentPlanStatus"><option value="open">المواعيد المفتوحة</option><option value="overdue">المتأخرة</option><option value="due_today">مستحقة اليوم</option><option value="upcoming">القادمة</option><option value="postponed">المؤجلة</option><option value="completed">تم السداد</option><option value="cancelled">الملغاة</option><option value="all">الكل</option></select><input class="input" type="week" id="paymentPlanWeek" title="الأسبوع"><button class="btn btn-soft btn-sm" id="paymentPlanAllWeeks" type="button">كل الأسابيع</button></div><div class="hint">اختار الأسبوع لعرض خطة السداد الأسبوعية، أو كل الأسابيع لعرض كل المواعيد.</div></section><div id="paymentPlanRows"></div>`;
  const branch=document.getElementById('paymentPlanBranch'),status=document.getElementById('paymentPlanStatus'),search=document.getElementById('paymentPlanSearch'),week=document.getElementById('paymentPlanWeek');branch.value=state.paymentPlanBranchId||'';status.value=state.paymentPlanStatus||'open';week.value=state.paymentPlanWeek||'';
  ...
  const refresh=()=>{state.paymentPlanBranchId=branch.value;state.paymentPlanStatus=status.value;state.paymentPlanSearch=search.value;state.paymentPlanWeek=week.value;renderPaymentPlanRows();};branch.oninput=refresh;status.oninput=refresh;search.oninput=refresh;week.oninput=refresh;renderPaymentPlanRows();
}
...
function visiblePaymentPlanRows(){
  const q=(state.paymentPlanSearch||'').trim().toLowerCase(),filter=state.paymentPlanStatus||'open';
  const scope=state.paymentPlans.filter(p=>(!state.paymentPlanBranchId||p.branch_id===state.paymentPlanBranchId)&&(!state.paymentPlanWeek||dateInIsoWeek(p.planned_date,state.paymentPlanWeek))&&(!q||String(p.supplier_name||'').toLowerCase().includes(q)));
...
function exportPaymentPlanPdf(){
  const rows=visiblePaymentPlanRows(),range=state.paymentPlanWeek?isoWeekRange(state.paymentPlanWeek):null,branch=state.paymentPlanBranchId?(state.branches.find(b=>b.id===state.paymentPlanBranchId)?.name||'فرع محدد'):'كل الفروع';
