...
const state = {
  ...
  employees: [], selectedEmployeeId: '', employeeRecords: [], employeeSearch: '', employeeMonth: isoToday().slice(0,7), employeeRecordType: 'absence', employeeBranchId: '',
  supplierBranchId: '',
  supplierCategoryId: '',
  ...
  movementReports: [], movementRows: [], movementReport: null, movementReportId: '', movementBranchId: '', movementSearch: '', movementStatus: 'all', movementAbc: 'all', movementSort: 'desc', movementPurchaseArchive: null, movementPurchaseSummary: null,
...
function isoWeekValue(dateValue=isoToday()){
  const d=new Date(`${dateValue}T12:00:00`);if(Number.isNaN(d.getTime()))return '';
  const tmp=new Date(Date.UTC(d.getFullYear(),d.getMonth(),d.getDate()));tmp.setUTCDate(tmp.getUTCDate()+4-(tmp.getUTCDay()||7));
  const yearStart=new Date(Date.UTC(tmp.getUTCFullYear(),0,1));const week=Math.ceil((((tmp-yearStart)/86400000)+1)/7);
  return `${tmp.getUTCFullYear()}-W${String(week).padStart(2,'0')}`;
}
function isoWeekRange(weekValue){
  const m=String(weekValue||'').match(/^(\d{4})-W(\d{2})$/);if(!m)return null;const year=Number(m[1]),week=Number(m[2]);
  const simple=new Date(Date.UTC(year,0,1+(week-1)*7));const dow=simple.getUTCDay()||7;const monday=new Date(simple);monday.setUTCDate(simple.getUTCDate()-dow+1);const sunday=new Date(monday);sunday.setUTCDate(monday.getUTCDate()+6);
  const fmt=d=>d.toISOString().slice(0,10);return {start:fmt(monday),end:fmt(sunday),label:`${fmt(monday)} → ${fmt(sunday)}`};
}
function dateInIsoWeek(dateValue,weekValue){const r=isoWeekRange(weekValue);const d=String(dateValue||'').slice(0,10);return !r||!d?true:(d>=r.start&&d<=r.end);}
function can(p){return !!state.profile?.effective_permissions?.[p];}
function canUseTasksHub(){return can('use_employee_records')||can('use_daily_notes')||can('use_tasks');}
...
async function logout(){
  ...
  clearSession();sessionExpiredShown=false;renderLogin();
}
...
async function loadBase(){
  ...
  if(can('view_dashboard') || can('view_suppliers') || can('view_payment_plans')) jobs.push(api('/api/admin/supplier-categories').then(x=>state.categories=x||[]));
...
async function renderLogin(){
  ...
  root.innerHTML=`<div class="login-shell"><div class="login-card">
    ...
    <form id="loginForm">
      <div class="field"><label>اسم المستخدم</label>${accounts.length?`<select class="select" name="username" required><option value="">اختر الحساب</option>${accounts.map(a=>`<option value="${esc(a.username)}">${esc(a.full_name||a.username)}</option>`).join('')}</select>`:`<input class="input" name="username" autocomplete="username" required>`}</div>
...
function renderApp(){
  ...
  const fallbackView=()=>can('view_dashboard')?'dashboard':canUseTasksHub()?'tasks':can('view_invoices')?'invoices':can('view_suppliers')?'suppliers':can('view_payments')?'payments':can('view_item_analysis')?'items':can('view_reports')?'reportbuilder':'settings';
  if((state.view==='tasks'&&!canUseTasksHub())||(allowedViews[state.view] && !can(allowedViews[state.view]))) state.view=fallbackView();
...
function renderPdfBarChart(rows,metric,title,subtitle='',limit=8,formatter=money){
  const clean=[...(rows||[])].filter(x=>Number(x[metric]||0)>0).sort((a,b)=>Number(b[metric]||0)-Number(a[metric]||0)).slice(0,limit);
  if(!clean.length)return `<section class="section pdf-chart-section"><h2 class="section-title">${esc(title)}</h2><div class="note-box">لا توجد بيانات كافية للرسم.</div></section>`;
  const max=Math.max(...clean.map(x=>Number(x[metric]||0)),1);
  return `<section class="section pdf-chart-section"><h2 class="section-title">${esc(title)}</h2>${subtitle?`<div class="note-box">${esc(subtitle)}</div>`:''}<div class="pdf-bar-chart">${clean.map((x,i)=>{const pct=Math.max(2,Math.min(100,(Number(x[metric]||0)/max)*100));return `<div class="pdf-bar-row"><span class="pdf-rank">${i+1}</span><strong class="pdf-bar-name">${esc(x.doctor||'—')}</strong><div class="pdf-bar-track"><span style="width:${pct.toFixed(2)}%"></span></div><em>${formatter(x[metric])}</em></div>`;}).join('')}</div></section>`;
}
function renderPdfDoctorShareChart(rows){
  const clean=[...(rows||[])].filter(x=>Number(x.net_sales||0)>0).sort((a,b)=>Number(b.net_sales||0)-Number(a.net_sales||0));
  const total=clean.reduce((s,x)=>s+Number(x.net_sales||0),0);if(total<=0)return '';
  const top=clean.slice(0,8);
  return `<section class="section pdf-chart-section"><h2 class="section-title">نسبة مساهمة الدكاترة من الصافي</h2><div class="pdf-bar-chart">${top.map((x,i)=>{const pct=(Number(x.net_sales||0)/total)*100;return `<div class="pdf-bar-row"><span class="pdf-rank">${i+1}</span><strong class="pdf-bar-name">${esc(x.doctor||'—')}</strong><div class="pdf-bar-track"><span style="width:${Math.max(2,pct).toFixed(2)}%"></span></div><em>${pct.toLocaleString('en-US',{minimumFractionDigits:1,maximumFractionDigits:1})}%</em></div>`;}).join('')}</div></section>`;
}
function renderReportBuilderPdfCharts(data){
  const rows=data?.doctors||[];if(!data||!rows.length)return '';
  return `${renderPdfDailySalesChart(buildPharmacyDailySalesRows(data),'المبيعات اليومية للصيدلية','أهم شارت في التقرير: حركة صافي المبيعات اليومية للصيدلية خلال الفترة المختارة.')}${renderPdfBarChart(rows,'net_sales','أعلى الدكاترة حسب صافي المبيعات','Top 8 حسب الصافي',8,money)}${renderPdfDoctorShareChart(rows)}${renderPdfBarChart(rows,'invoice_count','الأكثر عدد فواتير','Top 8 حسب عدد الفواتير',8,x=>Number(x||0).toLocaleString('en-US'))}${renderPdfBarChart(rows,'kpi_score','أعلى KPI للدكاترة','تقييم الأداء العام',8,x=>doctorKpiValue(x))}`;
}
...
function renderDoctorSalesResults(){
  ...
  ${renderPharmacyDailySalesChart(data)}
...
function openDoctorPdfPrintWindow({title,subtitle='',body='',orientation='portrait'}){
  ...
  *{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  .pdf-chart-section{break-inside:avoid;margin-bottom:11px}.pdf-chart-stats{display:flex;gap:6px;flex-wrap:wrap;margin:6px 0 7px}.pdf-chart-stats span{border:1px solid #d8e5e1;background:#f8fbfa;border-radius:999px;padding:4px 8px;font-size:8.5px}.pdf-daily-chart{display:block;border:1px solid #d9e5e1;border-radius:10px;background:#fff}.pdf-grid line{stroke:#e1ebe7;stroke-width:1}.pdf-y-label{direction:ltr;text-anchor:end;font-size:10px;fill:#65756f;font-variant-numeric:tabular-nums}.pdf-x-label{text-anchor:middle;font-size:9px;fill:#65756f}.pdf-bars rect{fill:#9fd8c4;stroke:#66b99e;stroke-width:.4}.pdf-line{fill:none;stroke:#0f7a5b;stroke-width:2.5;stroke-linecap:round;stroke-linejoin:round}.pdf-bar-chart{display:grid;gap:5px}.pdf-bar-row{display:grid;grid-template-columns:24px minmax(110px,1fr) 2.2fr 90px;gap:6px;align-items:center;border:1px solid #dce7e3;border-radius:8px;padding:5px 6px;break-inside:avoid}.pdf-rank{width:20px;height:20px;border-radius:50%;background:#e7f4ef;color:#0f654d;display:inline-flex;align-items:center;justify-content:center;font-weight:700;font-size:8px}.pdf-bar-name{font-size:9px;color:#213d35;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.pdf-bar-track{height:12px;background:#eef5f2;border-radius:999px;overflow:hidden}.pdf-bar-track span{display:block;height:100%;background:#0f7a5b;border-radius:999px}.pdf-bar-row em{direction:ltr;text-align:left;font-style:normal;font-weight:700;color:#0f654d;font-size:9px;font-variant-numeric:tabular-nums}
...
function exportDoctorSalesAnalysisPdf(){
  ...
  <section class="section"><div class="cards">
    ...
    <div class="card"><div class="label">متوسط KPI الفريق</div><div class="value">${doctorKpiValue(t.kpi_average)}</div></div><div class="card"><div class="label">متوسط الإيراد اليومي</div><div class="value">${money(t.daily_average)}</div></div>
  </div></section>
...
function invalidateItemSalesRates(){state.itemSalesRates={};state.itemSalesRateMeta=null;}
function catalogRateFor(item){return state.itemSalesRates?.[String(item?.id||'')]||null;}
function catalogRateSourceText(){
  const m=state.itemSalesRateMeta;if(!m)return 'جاري حساب معدل البيع المعتمد من تقارير الحركة المحفوظة...';
  if(!Number(m.effective_report_count||0))return 'لا توجد تقارير حركة محفوظة ضمن النطاق المختار.';
  const scope=m.branch_name||(!m.branch_id?'كل الفروع':'الفرع المحدد');
  return `${esc(scope)} · ${Number(m.effective_report_count||0).toLocaleString('en-US')} تقرير معتمد · ${esc(m.period_start||'—')} → ${esc(m.period_end||'—')} · ${Number(m.coverage_days||0).toLocaleString('en-US')} يوم`;
}
async function loadItemSalesRates(){
  const requestId=++state.itemSalesRateRequestSeq;
  try{
    const params=new URLSearchParams();if(state.itemRateBranchId)params.set('branch_id',state.itemRateBranchId);
    const data=await api(`/api/items/sales-rates${params.toString()?`?${params}`:''}`);
    if(requestId!==state.itemSalesRateRequestSeq)return;
    state.itemSalesRates={};(data.rows||[]).forEach(r=>{state.itemSalesRates[String(r.item_id||'')]=r;});state.itemSalesRateMeta=data.meta||{};
...
async function tasksView(main){
  const available=[];
  if(can('use_employee_records'))available.push('employees');
  if(can('use_daily_notes'))available.push('notes');
  if(can('use_tasks'))available.push('tasks');
  if(!available.length){main.innerHTML='<div class="empty">لا توجد صلاحية لأي قسم داخل مهامي.</div>';return;}
  if(!available.includes(state.tasksSection))state.tasksSection=available[0];
  if(can('use_tasks'))await loadTasks();else state.tasks=[];
...
function noteBookCardsHtml(rows){
  return rows.length?`<div class="note-book-grid">${rows.map(b=>`<article class="note-book-card" data-open-note-book="${esc(b.id)}"><div class="note-book-main"><div class="note-book-avatar">${esc(String(b.title||'م').trim().charAt(0)||'م')}</div><div class="note-book-copy"><h4>${esc(b.title)}</h4><span>آخر تحديث · ${esc(fmtDateTime(b.updated_at||b.created_at))}</span></div><span class="note-book-chevron" aria-hidden="true">‹</span></div><div class="note-book-actions"><button class="note-icon-btn" title="تعديل" aria-label="تعديل" data-edit-note-book="${esc(b.id)}">✎</button><button class="note-icon-btn danger" title="حذف" aria-label="حذف" data-delete-note-book="${esc(b.id)}">×</button></div></article>`).join('')}</div>`:'<div class="empty notes-empty">لا توجد عناوين بعد.<br><span>اضغط «+ موظف / عنوان» لبدء تسجيل الملاحظات.</span></div>';
}
...
function noteBookDetailHtml(book){
  ...
  return `<div class="note-summary-strip"><span><b>${stats.total}</b> ملاحظة</span><span><b>${stats.today}</b> اليوم</span><span class="${stats.pending?'has-pending':''}"><b>${stats.pending}</b> تحتاج متابعة</span></div><div id="dailyNoteRows">${rows.length?`<div class="daily-note-list">${rows.map(n=>dailyNoteCardHtml(n,book)).join('')}</div>`:'<div class="empty notes-empty">لا توجد ملاحظات حتى الآن.<br><span>أضف أول ملاحظة لهذا العنوان.</span></div>'}</div>`;
}
function dailyNoteCardHtml(n,book){return `<article class="daily-note-card ${n.followed_up?'done':''}"><div class="daily-note-top"><div class="daily-note-date"><strong>${esc(taskDate(n.note_date)||'بدون تاريخ')}</strong><span>${n.followed_up?'تمت المتابعة':'تحتاج متابعة'}</span></div><div class="task-badges">${taskPriorityBadge(n.priority)}</div></div><p>${esc(n.note_text)}</p><div class="daily-note-actions">${!n.followed_up?`<button class="note-action-primary" data-note-follow="${esc(n.id)}">✓ تمت المتابعة</button>`:''}<button class="note-action-link" data-note-edit="${esc(n.id)}">تعديل</button><button class="note-action-link danger" data-note-delete="${esc(n.id)}">حذف</button></div></article>`;}
...
async function loadEmployees(){
  const q=new URLSearchParams();if(state.employeeBranchId)q.set('branch_id',state.employeeBranchId);
  state.employees=await api(`/api/tasks/employees${q.toString()?`?${q.toString()}`:''}`);
}
function employeeBranchName(employee){return employee?.branches?.name||state.branches.find(b=>b.id===employee?.branch_id)?.name||(employee?.branch_id?'فرع':'غير محدد');}
function employeeReportBranchLabel(){return state.employeeBranchId?(state.branches.find(b=>b.id===state.employeeBranchId)?.name||'الفرع المحدد'):'كل الفروع المسموحة';}
...
async function exportEmployeesPdf(){
  ...
  popup.document.close();
  try{
    const month=state.employeeMonth||isoToday().slice(0,7),q=new URLSearchParams();if(month)q.set('month',month);if(state.employeeBranchId)q.set('branch_id',state.employeeBranchId);
    const reportData=await api(`/api/tasks/employees/report?${q.toString()}`);
    const reportEmployees=Array.isArray(reportData?.employees)?reportData.employees:employees;
    const allRecords=Array.isArray(reportData?.records)?reportData.records:[];
    const recordsByEmployee=new Map();
    allRecords.forEach(r=>{const id=String(r.employee_id||'');if(!recordsByEmployee.has(id))recordsByEmployee.set(id,[]);recordsByEmployee.get(id).push(r);});
    const grouped=reportEmployees.map(employee=>{
      const records=recordsByEmployee.get(String(employee.id))||[];
      return {employee,records,totals:employeeTotalsFor(employee,records)};
    });
    ...
    const summaryRows=grouped.map((x,i)=>`<tr><td>${i+1}</td><td class="name">${esc(x.employee.name||'—')}<br><small>${esc(employeeBranchName(x.employee))}</small></td><td class="money">${money(x.totals.base)}</td><td class="money plus">${money(x.totals.overtime)}</td><td class="money minus">${money(x.totals.absence)}</td><td class="money minus">${money(x.totals.withdrawal)}</td><td class="money minus">${money(x.totals.credit)}</td><td class="money net">${money(x.totals.net)}</td></tr>`).join('');
    const detailSections=grouped.map(({employee,records,totals},idx)=>{
      ...
      return `<section class="employee-detail ${idx?'detail-break':''}"><div class="employee-title"><div><span>الموظف</span><h2>${esc(employee.name||'—')}</h2><span>${esc(employeeBranchName(employee))}</span></div><div class="employee-net"><span>صافي المرتب</span><strong>${money(totals.net)}</strong></div></div><div class="mini-summary"><div><span>الأساسي</span><b>${money(totals.base)}</b></div><div><span>الإضافي</span><b class="plus">+ ${money(totals.overtime)}</b></div><div><span>الغياب</span><b class="minus">- ${money(totals.absence)}</b></div><div><span>السحوبات</span><b class="minus">- ${money(totals.withdrawal)}</b></div><div><span>الأجل</span><b class="minus">- ${money(totals.credit)}</b></div></div><table class="details"><thead><tr><th>#</th><th>التاريخ</th><th>النوع</th><th>الأيام / الساعات</th><th>القيمة</th><th>الملاحظة / البيان</th></tr></thead><tbody>${rows}</tbody></table></section>`;
    }).join('');
    const generated=typeof doctorPdfGeneratedAt==='function'?doctorPdfGeneratedAt():new Date().toLocaleString('ar-LY');
    const monthText=employeeMonthLabel(),branchText=employeeReportBranchLabel();
    popup.document.open();
...
async function renderEmployeesSection(){
  ...
  if(!state.employeeBranchId&&state.branches.length===1)state.employeeBranchId=String(state.branches[0].id);
  await loadEmployees();
  ...
  if(selected){
    ...
    const branch=document.getElementById('employeesBranchFilter');if(branch){branch.value=state.employeeBranchId||'';branch.onchange=async()=>{state.employeeBranchId=branch.value;state.selectedEmployeeId='';state.employeeRecords=[];await renderEmployeesSection();};}
...
function employeeModal(employee=null){
  const currentBranch=employee?.branch_id||state.employeeBranchId||(state.branches.length===1?String(state.branches[0].id):'');
  const branchRows=state.branches.filter(b=>b.active||b.id===employee?.branch_id);
  const branchOpts=branchRows.map(b=>`<option value="${esc(b.id)}" ${String(b.id)===String(currentBranch)?'selected':''}>${esc(b.name)}</option>`).join('');
  showModal(`${employee?'تعديل':'إضافة'} موظف`,`<form id="employeeForm" class="form-grid"><div class="field full"><label>اسم الموظف *</label><input class="input" name="name" required minlength="2" maxlength="160" value="${esc(employee?.name||'')}" placeholder="اسم الموظف"></div><div class="field"><label>الفرع *</label><select class="select" name="branch_id" required><option value="">— اختر الفرع —</option>${branchOpts}</select></div><div class="field"><label>المرتب الأساسي الشهري *</label><input class="input" type="number" name="base_salary" required min="0" step="0.01" inputmode="decimal" value="${esc(employee?.base_salary??'')}" placeholder="مثال: 1500"></div></form>`,async()=>{const f=document.getElementById('employeeForm');if(!f.reportValidity())return false;const fd=new FormData(f),payload={name:String(fd.get('name')||'').trim(),base_salary:Number(fd.get('base_salary')||0),branch_id:String(fd.get('branch_id')||'')};try{const saved=await api(employee?`/api/tasks/employees/${employee.id}`:'/api/tasks/employees',{method:employee?'PUT':'POST',body:JSON.stringify(payload)});toast('تم حفظ الموظف');state.employeeBranchId=payload.branch_id;if(employee)state.selectedEmployeeId=employee.id;else if(saved?.id)state.selectedEmployeeId=saved.id;await renderEmployeesSection();return true;}catch(e){toast(e.message,true);return false;}},{saveText:'حفظ الموظف'});
}
...
async function renderItemsTab(){
  ...
  <section class="panel catalog-rate-panel"><div class="field"><label>معدل البيع حسب</label><select class="select" id="itemRateBranch"><option value="">كل الفروع</option>${branchOptions(false,false)}</select></div><div id="itemRateSourceInfo" class="catalog-rate-source"><strong>معدل البيع المعتمد</strong><span>جاري الحساب من كل تقارير الحركة المحفوظة...</span></div></section>
  ...
  const rateBranch=document.getElementById('itemRateBranch');rateBranch.value=state.itemRateBranchId||'';rateBranch.onchange=async()=>{state.itemRateBranchId=rateBranch.value;invalidateItemSalesRates();await loadItemSalesRates();};
  ...
  await Promise.all([loadItemCatalog(false),loadItemSalesRates()]);
}
...
function renderItemRows(){
  ...
  box.innerHTML=rows.length?`<div class="table-wrap desktop-table"><table><thead><tr><th>الكود</th><th>اسم الصنف</th><th>وحدة البيع</th><th>فرط / علبة</th><th>معدل البيع / يوم</th><th>آخر حركة</th>${can('manage_item_catalog')?'<th>إجراءات</th>':''}</tr></thead><tbody>${rows.map(x=>{const rate=catalogRateFor(x);return `<tr><td><strong>${esc(x.item_code)}</strong></td><td>${esc(x.item_name)}</td><td>${esc(x.package_form||'—')}</td><td><strong>${Number(x.units_per_box||0).toLocaleString('en-US')}</strong></td><td><strong class="catalog-sales-rate">${movementNumber(rate?.daily_rate||0,3)}</strong>${rate?`<small class="catalog-rate-detail">${Number(rate.report_count||0)} تقرير · ${movementNumber(rate.total_equivalent_boxes||0,1)} علبة</small>`:'<small class="catalog-rate-detail">لا توجد حركة محفوظة</small>'}</td><td>${esc(rate?.last_movement_date||'—')}</td>${can('manage_item_catalog')?`<td>${actions(x)}</td>`:''}</tr>`}).join('')}</tbody></table></div>
  <div class="mobile-list">${rows.map(x=>{const rate=catalogRateFor(x);return `<div class="item-card"><div class="item-title"><span>${esc(x.item_name)}</span><strong>${esc(x.item_code)}</strong></div><div class="item-meta"><div><span>معدل البيع المعتمد / يوم</span><strong class="catalog-sales-rate">${movementNumber(rate?.daily_rate||0,3)}</strong></div><div><span>آخر حركة</span>${esc(rate?.last_movement_date||'—')}</div><div><span>وحدة البيع</span>${esc(x.package_form||'—')}</div><div><span>فرط / علبة</span><strong>${Number(x.units_per_box||0).toLocaleString('en-US')}</strong></div></div>${rate?`<div class="muted">محسوب من ${Number(rate.report_count||0)} تقرير ظهر فيه الصنف · ${movementNumber(rate.total_equivalent_boxes||0,1)} علبة مكافئة</div>`:''}${can('manage_item_catalog')?`<div class="item-actions">${actions(x)}</div>`:''}</div>`}).join('')}</div>`:'<div class="panel"><div class="empty">لا توجد أصناف مطابقة</div></div>';
  box.querySelectorAll('[data-item-edit]').forEach(b=>b.onclick=()=>itemModal(state.items.find(x=>x.id===b.dataset.itemEdit)));
  box.querySelectorAll('[data-item-delete]').forEach(b=>b.onclick=async()=>{if(!confirmAction('حذف الصنف من دليل الأصناف؟'))return;try{await api(`/api/items/${b.dataset.itemDelete}`,{method:'DELETE'});toast('تم حذف الصنف');invalidateItemSalesRates();await Promise.all([loadItemCatalog(false),loadItemSalesRates()]);}catch(e){toast(e.message,true);}});
}
...
async function renderMovementAnalysis(box){
  box.innerHTML=`<div class="page-head movement-head"><div><h3>تحليل حركة الأصناف</h3><div class="muted">المبيعات الشهرية + أرشيف المشتريات + ABC تعطيك قرار شراء أوضح.</div></div>${can('manage_item_catalog')?'<div class="page-head-actions"><button class="btn btn-soft" id="uploadPurchaseArchive">رفع مشتريات كاملة</button><button class="btn btn-primary" id="uploadMovementReport">+ رفع تقرير المبيعات والحركة</button></div>':''}</div>
  <section class="movement-filter-panel"><div class="movement-filters"><div class="field"><label>الفرع</label><select class="select" id="movementBranchFilter"><option value="">كل الفروع</option>${branchOptions(false,false)}</select></div><div class="field"><label>التقرير</label><select class="select" id="movementReportSelect"><option value="">جاري تحميل التقارير...</option></select></div><div class="field"><label>الحالة</label><select class="select" id="movementStatusFilter"><option value="all">كل الأصناف</option><option value="unmatched">تحتاج مطابقة</option><option value="blocking">غير محسوبة بسبب الفرط</option></select></div><div class="field"><label>تصنيف ABC</label><select class="select" id="movementAbcFilter"><option value="all">كل التصنيفات</option><option value="A">A فقط - الأصناف الأهم</option><option value="B">B فقط - متوسطة</option><option value="C">C فقط - قليلة</option></select></div><div class="field"><label>ترتيب معدل البيع</label><select class="select" id="movementSortFilter"><option value="desc">الأعلى إلى الأقل</option><option value="asc">الأقل إلى الأعلى</option></select></div></div><div class="toolbar movement-search-toolbar"><input class="input" id="movementSearch" value="${esc(state.movementSearch)}" placeholder="بحث باسم الصنف أو الكود..."></div></section>
  <div id="purchaseArchiveBox" class="purchase-archive-box"><div class="loading">جاري فحص أرشيف المشتريات...</div></div><div id="movementAnalysisBox"><div class="loading">جاري تحميل تقارير الحركة...</div></div>`;
  const branch=document.getElementById('movementBranchFilter'),status=document.getElementById('movementStatusFilter'),abc=document.getElementById('movementAbcFilter'),sort=document.getElementById('movementSortFilter'),search=document.getElementById('movementSearch');
  branch.value=state.movementBranchId||'';status.value=state.movementStatus||'all';abc.value=state.movementAbc||'all';sort.value=state.movementSort||'desc';
  branch.onchange=async()=>{state.movementBranchId=branch.value;state.movementReportId='';await loadPurchaseArchive();await loadMovementReports();};
  status.onchange=()=>{state.movementStatus=status.value;renderMovementRows();};
  abc.onchange=()=>{state.movementAbc=abc.value;renderMovementRows();};
  ...
  if(can('manage_item_catalog')){document.getElementById('uploadMovementReport').onclick=()=>movementUploadModal();document.getElementById('uploadPurchaseArchive').onclick=()=>purchaseArchiveModal();}
  await loadPurchaseArchive();
...
async function loadMovementReportDetail(){
  ...
  try{const data=await api(`/api/item-movements/reports/${state.movementReportId}`);state.movementReport=data.report||null;state.movementRows=data.rows||[];state.movementReport.total_equivalent_boxes=Number(data.total_equivalent_boxes||0);state.movementReport.blocking_count=Number(data.blocking_count||0);state.movementPurchaseSummary=null;await loadMovementPurchaseSummary();renderMovementRows();}catch(e){box.innerHTML=`<section class="panel"><div class="empty">${esc(e.message)}</div></section>`;toast(e.message,true);}
}

async function loadPurchaseArchive(){
  const box=document.getElementById('purchaseArchiveBox');if(!box)return;
  try{
    const q=new URLSearchParams();if(state.movementBranchId)q.set('branch_id',state.movementBranchId);
    const rows=await api(`/api/item-purchases/archive?${q.toString()}`)||[];
    state.movementPurchaseArchive=rows[0]||null;
    const a=state.movementPurchaseArchive;
    if(!a){box.innerHTML=`<section class="panel purchase-archive-panel"><div><strong>أرشيف المشتريات</strong><span class="muted">لا يوجد تقرير مشتريات كامل محفوظ لهذا الفرع.</span></div>${can('manage_item_catalog')?'<button class="btn btn-soft btn-sm" id="purchaseArchiveQuickUpload">رفع مشتريات كاملة</button>':''}</section>`;document.getElementById('purchaseArchiveQuickUpload')?.addEventListener('click',()=>purchaseArchiveModal());return;}
    const branch=(a.branches||{}).name||'الفرع المحدد';
    box.innerHTML=`<section class="panel purchase-archive-panel"><div><strong>أرشيف المشتريات محفوظ</strong><span>${esc(branch)} · ${esc(a.period_start)} → ${esc(a.period_end)}</span><small>${Number(a.row_count||0).toLocaleString('en-US')} سطر · ${Number(a.unique_item_count||0).toLocaleString('en-US')} صنف · ${movementNumber(a.total_equivalent_boxes||0,2)} علبة مكافئة</small></div><div class="actions"><button class="btn btn-soft btn-sm" id="refreshPurchaseArchive">تحديث العرض</button>${can('manage_item_catalog')?`<button class="btn btn-danger btn-sm" id="deletePurchaseArchive">حذف الأرشيف</button>`:''}</div></section>`;
    document.getElementById('refreshPurchaseArchive')?.addEventListener('click',loadPurchaseArchive);
    document.getElementById('deletePurchaseArchive')?.addEventListener('click',async()=>{if(!confirmAction('حذف أرشيف المشتريات المحفوظ لهذا الفرع؟'))return;try{await api(`/api/item-purchases/archive/${a.id}`,{method:'DELETE'});state.movementPurchaseArchive=null;state.movementPurchaseSummary=null;toast('تم حذف أرشيف المشتريات');await loadPurchaseArchive();if(state.movementReport)renderMovementRows();}catch(e){toast(e.message,true);}});
  }catch(e){box.innerHTML=`<section class="panel purchase-archive-panel"><div class="empty">${esc(e.message)}</div></section>`;}
}
async function loadMovementPurchaseSummary(){
  const r=state.movementReport;if(!r||!r.branch_id||!r.period_start||!r.period_end)return;
  try{
    const q=new URLSearchParams({branch_id:r.branch_id,start:r.period_start,end:r.period_end});
    const data=await api(`/api/item-purchases/summary?${q.toString()}`);
    state.movementPurchaseSummary=data||null;
    const map={};(data.rows||[]).forEach(x=>{if(x.item_id)map[`item:${x.item_id}`]=x;if(x.report_name_norm)map[`name:${x.report_name_norm}`]=x;});
...
function movementTurnoverText(row){
  const sold=Number(row?.equivalent_boxes||0), purchased=Number(movementPurchaseFor(row)?.equivalent_boxes||0), diff=purchased-sold, rate=Number(row?.daily_rate||0), last=movementPurchaseFor(row)?.last_purchase_date||'';
  if(sold>0&&purchased<=0)return 'بيع بدون مشتريات بالفترة';
  if(sold<=0&&purchased>0)return 'مشتريات بلا بيع';
  if(rate>=1&&diff<0)return 'سريع ومحتاج متابعة';
  if(rate>=1&&diff>=0)return last?'سريع ومشتراه':'سريع';
  if(purchased>sold*2&&purchased>0)return 'شراء زائد عن البيع';
  if(sold>0&&diff<0)return 'المباع أكثر من المشترى';
  return 'طبيعي';
}
function movementDecorateAbc(rows){
  const ranked=[...rows].sort((a,b)=>Number(b.equivalent_boxes||0)-Number(a.equivalent_boxes||0));
  const total=ranked.reduce((sum,x)=>sum+Math.max(0,Number(x.equivalent_boxes||0)),0);
  let acc=0;ranked.forEach(x=>{const val=Math.max(0,Number(x.equivalent_boxes||0));acc+=val;const ratio=total>0?acc/total:1;x.abc_class=ratio<=0.80?'A':(ratio<=0.95?'B':'C');x.abc_share=total>0?val/total:0;x.abc_cumulative=ratio;});
  return rows;
}
function movementAbcBadge(row){const c=row?.abc_class||'C';const labels={A:'A - أهم 80%',B:'B - حتى 95%',C:'C - باقي الأصناف'};return `<span class="abc-badge abc-${esc(c)}">${esc(labels[c]||c)}</span>`;}
function movementFilteredSortedRows(){
  const q=(state.movementSearch||'').trim().toLowerCase(),status=state.movementStatus||'all',abc=state.movementAbc||'all';
  const decorated=movementDecorateAbc([...(state.movementRows||[])]);
  const rows=decorated.filter(x=>{const cat=x.item_catalog||{};const text=`${x.report_name||''} ${cat.item_name||''} ${cat.item_code||''}`.toLowerCase();if(q&&!text.includes(q))return false;if(status==='unmatched'&&x.item_id)return false;if(status==='blocking'&&(x.item_id||Number(x.loose_sold||0)<=0))return false;if(abc!=='all'&&x.abc_class!==abc)return false;return true;});
...
function movementAbcText(){return ({all:'كل التصنيفات',A:'A فقط - الأصناف الأهم',B:'B فقط - متوسطة',C:'C فقط - قليلة'})[state.movementAbc||'all']||'كل التصنيفات';}
function movementAbcQuickFilters(counts={}){
  const current=state.movementAbc||'all';
  const items=[['all','الكل',(Number(counts.A||0)+Number(counts.B||0)+Number(counts.C||0))],['A','A',counts.A||0],['B','B',counts.B||0],['C','C',counts.C||0]];
  return `<div class="movement-abc-quick"><span>فلترة ABC</span>${items.map(([v,label,count])=>`<button type="button" class="abc-filter-btn ${current===v?'active':''}" data-movement-abc-filter="${esc(v)}"><b>${esc(label)}</b><small>${Number(count||0).toLocaleString('en-US')}</small></button>`).join('')}</div>`;
}
function exportMovementReportPdf(){
  ...
  const exportedPurchased=rows.reduce((sum,x)=>sum+Number(movementPurchaseFor(x)?.equivalent_boxes||0),0);
  const abcCounts=rows.reduce((m,x)=>{m[x.abc_class||'C']=(m[x.abc_class||'C']||0)+1;return m;},{});
  const rowsHtml=rows.map(x=>{const cat=x.item_catalog||{};const p=movementPurchaseFor(x)||{};const name=x.report_name||cat.item_name||'—';return `<tr><td class="item-name"><strong>${esc(name)}</strong><small>${esc(movementTurnoverText(x))}</small></td><td>${esc(x.abc_class||'C')}</td><td>${movementNumber(x.equivalent_boxes,2)}</td><td>${movementNumber(p.equivalent_boxes||0,2)}</td><td>${movementNumber(movementPurchaseDiff(x),2)}</td><td>${esc(p.last_purchase_date||'—')}</td><td>${movementNumber(p.last_purchase_equivalent_boxes||0,2)}</td><td class="rate">${movementNumber(x.daily_rate,3)}</td></tr>`;}).join('');
  popup.document.open();
  popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="UTF-8"><title>تقرير حركة الأصناف - ${esc(branch)}</title><style>@page{size:A4 landscape;margin:9mm}*{box-sizing:border-box}body{font-family:Arial,Tahoma,sans-serif;color:#173b36;margin:0;background:#fff;direction:rtl}.head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #0f6259;padding-bottom:8px;margin-bottom:10px}.head h1{margin:0 0 4px;font-size:20px;color:#0f6259}.meta{font-size:10px;line-height:1.8;color:#536a66}.summary{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin:8px 0 10px}.card{border:1px solid #d7e2df;border-radius:8px;padding:7px;text-align:center}.card span{display:block;font-size:8px;color:#6e817d}.card strong{display:block;margin-top:2px;font-size:12px;color:#173b36}.filters{font-size:9px;background:#f3f8f7;border:1px solid #d7e2df;border-radius:7px;padding:6px 8px;margin-bottom:8px}table{width:100%;border-collapse:collapse;font-size:8px;table-layout:fixed}thead{display:table-header-group}th{background:#0f6259;color:#fff;font-weight:700;padding:5px 4px;border:1px solid #0d574f}td{padding:4px;border:1px solid #dce5e3;text-align:center;vertical-align:middle;word-break:break-word}tbody tr:nth-child(even){background:#f7faf9}.item-name{text-align:right;width:44%}.item-name strong{display:block}.rate{font-weight:700;color:#0f6259}.foot{font-size:8px;color:#75837f;margin-top:7px;display:flex;justify-content:space-between}.no-print{margin:10px 0;text-align:center}@media print{.no-print{display:none}body{-webkit-print-color-adjust:exact;print-color-adjust:exact}}</style></head><body><div class="head"><div><h1>تقرير حركة الأصناف</h1><div class="meta">الفرع: <strong>${esc(branch)}</strong><br>الفترة: ${esc(r.period_start)} → ${esc(r.period_end)} (${Number(r.days_count||0)} يوم)</div></div><div class="meta">Abdo Debts<br>تاريخ التصدير: ${esc(generated)}</div></div><div class="summary"><div class="card"><span>عدد الأصناف</span><strong>${rows.length.toLocaleString('en-US')}</strong></div><div class="card"><span>A / B / C</span><strong>${Number(abcCounts.A||0)} / ${Number(abcCounts.B||0)} / ${Number(abcCounts.C||0)}</strong></div><div class="card"><span>إجمالي المباع</span><strong>${movementNumber(exportedEquivalent,2)}</strong></div><div class="card"><span>إجمالي المشترى</span><strong>${movementNumber(exportedPurchased,2)}</strong></div><div class="card"><span>فرق شراء/بيع</span><strong>${movementNumber(exportedPurchased-exportedEquivalent,2)}</strong></div></div><div class="filters">الحالة: <strong>${esc(movementStatusText())}</strong> · ABC: <strong>${esc(movementAbcText())}</strong> · ترتيب معدل البيع: <strong>${esc(sortText)}</strong>${state.movementSearch?` · البحث: <strong>${esc(state.movementSearch)}</strong>`:''}</div><table><thead><tr><th style="width:32%">الصنف</th><th>ABC</th><th>مباع علب</th><th>مشتريات</th><th>فرق</th><th>آخر شراء</th><th>كمية آخر شراء</th><th>معدل/يوم</th></tr></thead><tbody>${rowsHtml}</tbody></table><div class="foot"><span>ABC محسوب من إجمالي العلب المباعة: A حتى 80%، B حتى 95%، C الباقي</span><span>${rows.length.toLocaleString('en-US')} صنف</span></div><div class="no-print"><button id="movementPrintButton" style="font:inherit;padding:8px 18px;border:0;border-radius:8px;background:#0f6259;color:#fff;cursor:pointer">حفظ كـ PDF / طباعة</button></div></body></html>`);
  popup.document.close();
...
function renderMovementRows(){
  ...
  const unresolved=Number(r.unresolved_count||0),blocking=Number(r.blocking_count||0),branch=(r.branches||{}).name||'',pSummary=state.movementPurchaseSummary?.summary||{},pError=state.movementPurchaseSummary?.error||'';
  const allAbc=movementDecorateAbc([...(state.movementRows||[])]);const abcCounts=allAbc.reduce((m,x)=>{m[x.abc_class||'C']=(m[x.abc_class||'C']||0)+1;return m;},{});
  box.innerHTML=`<div class="movement-summary cards"><div class="stat"><div class="label">الفترة</div><div class="value movement-period-value">${esc(r.period_start)} → ${esc(r.period_end)}</div><div class="sub">${Number(r.days_count||0)} يوم</div></div><div class="stat"><div class="label">عدد الأصناف</div><div class="value">${Number(r.unique_item_count||0).toLocaleString('en-US')}</div><div class="sub">${Number(r.transaction_count||0).toLocaleString('en-US')} حركة بيع</div></div><div class="stat"><div class="label">إجمالي العلب المكافئة</div><div class="value">${movementNumber(r.total_equivalent_boxes,2)}</div><div class="sub">للأصناف المحسوبة</div></div><div class="stat"><div class="label">ABC</div><div class="value">A ${Number(abcCounts.A||0)}</div><div class="sub">B ${Number(abcCounts.B||0)} · C ${Number(abcCounts.C||0)}</div></div><div class="stat"><div class="label">مشتريات نفس الفترة</div><div class="value">${pError?'—':movementNumber(pSummary.total_equivalent_boxes||0,2)}</div><div class="sub">${pError?esc(pError):`${Number(pSummary.row_count||0).toLocaleString('en-US')} سطر مشتريات`}</div></div><div class="stat ${unresolved?'movement-warning-stat':''}"><div class="label">تحتاج مطابقة</div><div class="value">${unresolved.toLocaleString('en-US')}</div><div class="sub">${blocking?`${blocking} منها تمنع حساب الفرط`:'كل المعدلات محسوبة'}</div></div></div>
  <section class="panel movement-report-info"><div><strong>${esc(branch)}</strong><span>${movementPeriod(r)}</span>${r.source_name?`<small>المصدر: ${esc(r.source_name)}</small>`:''}${state.movementPurchaseSummary?.archive?`<small>المشتريات المحفوظة: ${esc(state.movementPurchaseSummary.archive.period_start)} → ${esc(state.movementPurchaseSummary.archive.period_end)}</small>`:''}</div><div class="actions">${unresolved?`<button class="btn btn-soft btn-sm" id="showUnmatchedMovement">إظهار غير المطابق (${unresolved})</button>`:''}<button class="btn btn-soft btn-sm" id="exportMovementPdf">تصدير PDF</button>${can('manage_item_catalog')?`<button class="btn btn-danger btn-sm" id="deleteMovementReport">حذف التقرير</button>`:''}</div></section>
  ${movementAbcQuickFilters(abcCounts)}
  <div class="movement-results-head"><span>عرض ${rows.length.toLocaleString('en-US')} من ${state.movementRows.length.toLocaleString('en-US')} صنف</span><small>ABC: ${esc(movementAbcText())} · معدل البيع = إجمالي العلب المكافئة ÷ ${Number(r.days_count||0)} يوم · ${state.movementSort==='asc'?'الأقل إلى الأعلى':'الأعلى إلى الأقل'}</small></div>
  ${rows.length?`<div class="table-wrap desktop-table"><table class="movement-table"><thead><tr><th>الصنف</th><th>ABC</th><th>إجمالي البيع</th><th>مشتريات الفترة</th><th>آخر شراء</th><th>متوسط شراء</th><th>فرق شراء/بيع</th><th>قرار</th><th>معدل/يوم</th><th>المطابقة</th></tr></thead><tbody>${rows.map(x=>{const cat=x.item_catalog||{};return `<tr><td><strong>${esc(x.report_name)}</strong>${cat.item_name&&cat.item_name!==x.report_name?`<small>${esc(cat.item_name)}</small>`:''}${cat.item_code?`<small>كود: ${esc(cat.item_code)}</small>`:''}</td><td>${movementAbcBadge(x)}</td><td><strong>${movementNumber(x.equivalent_boxes,2)}</strong><small>${movementNumber(x.boxes_sold,2)} علبة · ${movementNumber(x.loose_sold,2)} فرط</small></td><td><strong>${movementNumber(movementPurchaseFor(x)?.equivalent_boxes||0,2)}</strong><small>${Number(movementPurchaseFor(x)?.purchase_count||movementPurchaseFor(x)?.line_count||0).toLocaleString('en-US')} مرات</small></td><td>${esc(movementPurchaseFor(x)?.last_purchase_date||'—')}<small>${movementNumber(movementPurchaseFor(x)?.last_purchase_equivalent_boxes||0,2)} علبة</small></td><td>${movementNumber(movementPurchaseFor(x)?.average_purchase_equivalent_boxes||0,2)}</td><td><strong class="${movementPurchaseDiff(x)<0?'text-danger':'movement-rate'}">${movementNumber(movementPurchaseDiff(x),2)}</strong></td><td><span class="movement-decision">${esc(movementTurnoverText(x))}</span></td><td><strong class="movement-rate">${movementNumber(x.daily_rate,3)}</strong></td><td>${movementMatchBadge(x)}${!x.item_id&&can('manage_item_catalog')?`<div class="movement-row-actions"><button class="btn btn-ghost btn-sm movement-map-btn" data-map-movement="${x.id}">مطابقة</button><button class="btn btn-soft btn-sm" data-add-movement-catalog="${x.id}">+ إضافة للدليل</button></div>`:''}</td></tr>`}).join('')}</tbody></table></div>
  <div class="mobile-list">${rows.map(x=>{const cat=x.item_catalog||{};return `<div class="item-card movement-card"><div class="item-title"><span>${esc(x.report_name)}</span><strong>${movementNumber(x.daily_rate,3)}/يوم</strong></div>${cat.item_name?`<div class="muted movement-catalog-name">${esc(cat.item_name)}${cat.item_code?` · ${esc(cat.item_code)}`:''}</div>`:''}<div class="item-meta"><div><span>ABC</span><b>${movementAbcBadge(x)}</b></div><div><span>إجمالي البيع</span><b>${movementNumber(x.equivalent_boxes,2)}</b></div><div><span>مشتريات الفترة</span><b>${movementNumber(movementPurchaseFor(x)?.equivalent_boxes||0,2)}</b></div><div><span>آخر شراء</span><b>${esc(movementPurchaseFor(x)?.last_purchase_date||'—')}</b></div><div><span>كمية آخر شراء</span><b>${movementNumber(movementPurchaseFor(x)?.last_purchase_equivalent_boxes||0,2)}</b></div><div><span>متوسط شراء</span><b>${movementNumber(movementPurchaseFor(x)?.average_purchase_equivalent_boxes||0,2)}</b></div><div><span>فرق شراء/بيع</span><b>${movementNumber(movementPurchaseDiff(x),2)}</b></div><div><span>قرار</span><b>${esc(movementTurnoverText(x))}</b></div></div><div class="item-actions">${movementMatchBadge(x)}${!x.item_id&&can('manage_item_catalog')?`<button class="btn btn-ghost btn-sm" data-map-movement="${x.id}">مطابقة بصنف موجود</button><button class="btn btn-soft btn-sm" data-add-movement-catalog="${x.id}">+ إضافة إلى دليل الأصناف</button>`:''}</div></div>`}).join('')}</div>`:'<section class="panel"><div class="empty">لا توجد أصناف مطابقة للفلتر الحالي</div></section>'}`;
  ...
  box.querySelectorAll('[data-movement-abc-filter]').forEach(btn=>btn.onclick=()=>{state.movementAbc=btn.dataset.movementAbcFilter||'all';const sel=document.getElementById('movementAbcFilter');if(sel)sel.value=state.movementAbc;renderMovementRows();});
  document.getElementById('exportMovementPdf')?.addEventListener('click',exportMovementReportPdf);
  document.getElementById('deleteMovementReport')?.addEventListener('click',async()=>{if(!confirmAction('حذف تقرير الحركة الحالي؟'))return;try{await api(`/api/item-movements/reports/${r.id}`,{method:'DELETE'});invalidateItemSalesRates();toast('تم حذف تقرير الحركة وتحديث معدل البيع المعتمد');state.movementReportId='';await loadMovementReports();}catch(e){toast(e.message,true);}});
...
function shortagesDraftFor(row){
  const key=String(row.item_id||row.movement_row_id||row.item_code||row.item_name||'');
...
function shortageCopyList(){
  ...
  const analysis=state.shortagesAnalysis,period=analysis?.rate_source?`${analysis.rate_source.period_start||'—'} → ${analysis.rate_source.period_end||'—'}`:'—';
  const lines=[`النواقص المقترحة — تغطية ${Number(analysis?.target_days||0)} يوم`,`معدل البيع المعتمد: كل تقارير الحركة المحفوظة (${Number(analysis?.rate_source?.effective_report_count||0)} تقرير)`,`فترة البيانات: ${period}`,`تاريخ المخزون: ${analysis?.stock_report?.report_date||'—'}`,''];
...
function renderShortageRows(){
  ...
  <section class="panel shortage-source-info"><div><strong>${esc(branch)}</strong><span>معدل البيع المعتمد: ${Number(report.report_count||0).toLocaleString('en-US')} تقرير حركة محفوظ · ${esc(report.period_start||'—')} → ${esc(report.period_end||'—')} · ${Number(report.days_count||0).toLocaleString('en-US')} يوم بيانات</span><span>المخزون: ${esc(stock.report_date||'تاريخ غير مقروء')} · ${Number(stock.stock_item_count||0).toLocaleString('en-US')} صنف</span>${Number(data.rate_source?.skipped_rows||0)?`<span class="shortage-rate-warning">⚠️ ${Number(data.rate_source.skipped_rows).toLocaleString('en-US')} سطر حركة غير مرتبط بدليل الأصناف ولم يدخل في المعدل.</span>`:''}</div><small>الكمية المقترحة = (معدل البيع المعتمد من كل التقارير × ${Number(data.target_days||0)} يوم) − الرصيد الحالي. إضافة أو حذف تقرير حركة تغيّر المعدل تلقائيًا.</small></section>
...
async function analyzeShortages(){
  const branchId=String(document.getElementById('shortagesBranch')?.value||state.shortagesBranchId||''),target=Math.max(1,Math.min(180,Math.round(Number(document.getElementById('shortagesTargetDays')?.value||state.shortagesTargetDays||14))));
  if(!branchId){toast('اختر الفرع أولًا حتى نستخدم معدلات البيع الخاصة به',true);return;}if(!state.shortagesFile){toast('ارفع تقرير المخزون المتوفر أولًا',true);return;}
  state.shortagesBranchId=branchId;state.shortagesTargetDays=target;const btn=document.getElementById('shortagesAnalyze'),box=document.getElementById('shortagesResults');btn.disabled=true;btn.textContent='جاري التحليل...';box.innerHTML='<div class="loading">جاري حساب معدل البيع المعتمد من كل التقارير وربطه بالمخزون...</div>';
  try{const fd=new FormData();fd.append('branch_id',branchId);fd.append('target_days',String(target));fd.append('file',state.shortagesFile);const data=await api('/api/shortages/analyze',{method:'POST',body:fd});state.shortagesAnalysis=data;state.shortagesDraft={};(data.rows||[]).forEach(row=>{const key=String(row.item_id||row.item_code||row.item_name||'');state.shortagesDraft[key]={selected:Number(row.suggested_quantity||0)>0,qty:Number(row.suggested_quantity||0)};});renderShortageRows();toast(`تم اقتراح ${Number(data.summary?.shortage_count||0).toLocaleString('en-US')} صنف ناقص اعتمادًا على ${Number(data.rate_source?.effective_report_count||0)} تقرير حركة`);}catch(e){box.innerHTML=`<section class="panel"><div class="empty">${esc(e.message)}</div></section>`;toast(e.message,true);}finally{btn.disabled=false;btn.textContent=state.shortagesAnalysis?'إعادة تحليل النواقص':'تحليل النواقص';}
}
async function shortagesView(main){
  if(!state.shortagesBranchId){state.shortagesBranchId=state.movementBranchId||(state.branches.length===1?String(state.branches[0].id):'');}
  main.innerHTML=`<div class="page-head shortages-head"><div><h2>📦 النواقص المقترحة</h2><div class="muted">ارفع المخزون الحالي فقط، والبرنامج يستخدم معدل البيع المعتمد من جميع تقارير الحركة المحفوظة للفرع.</div></div><div class="page-head-actions"><button class="btn btn-soft" id="shortagesOpenMovement">فتح تحليل الحركة</button></div></div>
  <section class="panel shortage-setup"><div class="shortage-how"><strong>طريقة الحساب الذكية</strong><span>كل تقارير الحركة المحفوظة → معدل بيع معتمد لكل صنف في دليل الأصناف → ربط بالمخزون الحالي → أيام التغطية والكمية المقترحة.</span></div><div class="shortage-setup-grid"><div class="field"><label>الفرع *</label><select class="select" id="shortagesBranch" required><option value="">— اختر الفرع —</option>${branchOptions(false,false)}</select><div class="hint">النواقص تستخدم معدل البيع الخاص بالفرع المحدد فقط.</div></div><div class="field"><label>تقرير المخزون المتوفر *</label><input class="input" id="shortagesStockFile" type="file" accept=".csv,.xls,.xlsx,text/csv,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"><div class="hint" id="shortagesFileName">${state.shortagesFileName?`الملف الحالي: ${esc(state.shortagesFileName)}`:'ارفع التقرير كما يخرج من منظومة المخزون بدون تعديل.'}</div></div><div class="field"><label>التغطية المستهدفة (يوم) *</label><input class="input" id="shortagesTargetDays" type="number" min="1" max="180" step="1" value="${Number(state.shortagesTargetDays||14)}"><div class="shortage-presets"><button type="button" data-days="7">7</button><button type="button" data-days="14">14</button><button type="button" data-days="21">21</button><button type="button" data-days="30">30</button></div></div><div class="catalog-rate-source shortage-auto-rate"><strong>معدل البيع</strong><span>ما عادش تحتاج تختار تقرير حركة واحد. البرنامج يجمع كل التقارير المحفوظة تلقائيًا ويمنع تكرار الفترات المتداخلة قدر الإمكان.</span></div></div><div class="shortage-analyze-row"><button class="btn btn-primary" id="shortagesAnalyze">${state.shortagesAnalysis?'إعادة تحليل النواقص':'تحليل النواقص'}</button><span class="hint">كل تقرير حركة جديد تحفظه يدخل تلقائيًا في معدل البيع المعتمد.</span></div></section>
  <div id="shortagesResults">${state.shortagesAnalysis?'<div class="loading">جاري عرض آخر تحليل...</div>':'<section class="panel"><div class="empty">اختر الفرع، ارفع المخزون الحالي، وحدد مدة التغطية ثم اضغط «تحليل النواقص».</div></section>'}</div>`;
  const branch=document.getElementById('shortagesBranch'),file=document.getElementById('shortagesStockFile'),target=document.getElementById('shortagesTargetDays');branch.value=state.shortagesBranchId||'';
...
async function loadReportDrafts(){
 try{state.reportBuilderDrafts=await api('/api/report-drafts');}catch(e){state.reportBuilderDrafts=[];}
}
function applyReportDraft(d){
 state.reportBuilderDraftId=d.id||null;state.reportBuilderPreparedBy=d.prepared_by||'';state.reportBuilderAbsences=d.absences||'';state.reportBuilderProblems=d.problems||'';state.reportBuilderNotes=d.notes||'';state.reportBuilderAchievements=d.achievements||'';state.reportBuilderPlan=d.next_plan||'';
 reportBuilderView(document.getElementById('main'));toast('تم فتح المسودة');
}
async function deleteReportDraft(id){
 if(!confirm('حذف هذه المسودة؟'))return;
 try{await api(`/api/report-drafts/${id}`,{method:'DELETE'});await loadReportDrafts();reportBuilderView(document.getElementById('main'));toast('تم حذف المسودة');}catch(e){toast(e.message,true);}
}

async function reportBuilderView(main){
  if(!state.reportBuilderDrafts?.length) await loadReportDrafts();
  ...
  document.getElementById('refreshDrafts').onclick=async()=>{await loadReportDrafts();reportBuilderView(main);};
  main.querySelectorAll('[data-open-draft]').forEach(b=>b.onclick=()=>applyReportDraft((state.reportBuilderDrafts||[]).find(x=>String(x.id)===String(b.dataset.openDraft))));
  main.querySelectorAll('[data-delete-draft]').forEach(b=>b.onclick=()=>deleteReportDraft(b.dataset.deleteDraft));
...
function exportReportBuilderPdf(){
 ...
 const doctorRows=doctors.map((x,i)=>`<tr><td>${i+1}</td><td class="name">${esc(x.doctor)}</td><td>${doctorKpiValue(x.kpi_score)}</td><td>${Number(x.invoice_count||0).toLocaleString('en-US')}</td><td>${money(x.net_sales)}</td><td>${money(x.average_invoice)}</td><td>${money(x.daily_average)}</td></tr>`).join('');
 ...
 const qualityRows=doctors.map((x,i)=>`<tr><td>${i+1}</td><td class="name">${esc(x.doctor)}</td><td>${movementNumber(x.average_items_per_invoice||0,2)}</td><td>${Number(x.unique_items||0)}</td><td>${Number(x.high_value_invoice_percentage||0).toFixed(1)}%</td><td>${x.stability_score==null?'—':Number(x.stability_score).toFixed(1)+'%'}</td></tr>`).join('');
 const shortageRows=(state.shortagesAnalysis?.rows||[]).filter(x=>Number(x.suggested_quantity||0)>0).slice(0,30).map((x,i)=>`<tr><td>${i+1}</td><td class="name">${esc(x.item_name||'—')}</td><td>${movementNumber(x.stock_quantity||0,1)}</td><td>${movementNumber(x.daily_rate||0,2)}</td><td>${Number(x.suggested_quantity||0)}</td></tr>`).join('');
 const compare=state.periodComparePrevious&&state.periodCompareCurrent?(()=>{
   const old=state.periodComparePrevious,newer=state.periodCompareCurrent;
   const metrics=[['net_sales','صافي المبيعات'],['invoice_count','عدد الفواتير'],['active_days','أيام النشاط'],['daily_average','المبيعات / يوم نشاط'],['invoices_per_active_day','فواتير / يوم نشاط'],['average_invoice','متوسط الفاتورة'],['median_invoice','Median الفاتورة'],['high_value_invoice_percentage','نسبة الفواتير فوق 100'],['unique_items','الأصناف المختلفة'],['kpi_average','متوسط KPI الفريق']];
   const pharmacyRows=metrics.map(m=>{const a=m[0]==='kpi_average'?old.totals?.kpi_average:old.totals?.[m[0]],b=m[0]==='kpi_average'?newer.totals?.kpi_average:newer.totals?.[m[0]];return `<tr><td>${m[1]}</td><td>${a??'—'}</td><td>${b??'—'}</td><td>${a!=null&&b!=null?(Number(b)-Number(a)).toFixed(2):'—'}</td></tr>`}).join('');
   const doctors=periodCompareDoctorRows().map((x,i)=>`<tr><td>${i+1}</td><td class="name">${esc(x.doctor)}</td><td>${x.previous?doctorKpiValue(x.previous.kpi_score):'—'}</td><td>${x.current?doctorKpiValue(x.current.kpi_score):'—'}</td><td>${x.improvement_score==null?'—':x.improvement_score.toFixed(1)+'%'}</td><td>${x.previous?money(x.previous.net_sales):'—'}</td><td>${x.current?money(x.current.net_sales):'—'}</td><td>${x.previous&&x.current?periodCompareChangeText(x.previous.daily_average,x.current.daily_average):'—'}</td></tr>`).join('');
   return `<div class="note-box">مقارنة الفترة: ${esc(periodComparePeriodLabel(old))} مقابل ${esc(periodComparePeriodLabel(newer))}</div><h3>مقارنة الصيدلية ككل</h3><table><thead><tr><th>المؤشر</th><th>السابقة</th><th>الحالية</th><th>التغير</th></tr></thead><tbody>${pharmacyRows}</tbody></table><h3>مقارنة الدكاترة بالتفصيل</h3><table><thead><tr><th>#</th><th>الدكتور</th><th>KPI سابق</th><th>KPI حالي</th><th>مؤشر التحسن</th><th>صافي سابق</th><th>صافي حالي</th><th>تغير يومي</th></tr></thead><tbody>${doctors}</tbody></table>`;
 })():'<div class="note-box">لا توجد مقارنة فترة محملة.</div>';
 const body=`<section class="section"><h2 class="section-title">تحليل مبيعات الدكاترة</h2><div class="note-box">الفترة: ${esc(d.period_start||'—')} ← ${esc(d.period_end||'—')}<br>إعداد التقرير: ${esc(state.reportBuilderPreparedBy||'—')}</div><div class="cards"><div class="card"><div class="label">صافي المبيعات</div><div class="value">${money(d.totals.net_sales)}</div></div><div class="card"><div class="label">إجمالي المبيعات</div><div class="value">${money(d.totals.sales_total)}</div></div><div class="card"><div class="label">المرتجعات</div><div class="value">${money(d.totals.returns_total)}</div></div><div class="card"><div class="label">الفواتير</div><div class="value">${Number(d.totals.invoice_count||0)}</div></div><div class="card"><div class="label">متوسط KPI</div><div class="value">${doctorKpiValue(d.totals.kpi_average)}</div></div><div class="card"><div class="label">متوسط الإيراد اليومي</div><div class="value">${money(d.totals.daily_average)}</div></div></div></section>${renderReportBuilderPdfCharts(d)}<section class="section"><h2 class="section-title">تحليل الدكاترة و KPI + جودة الأداء</h2><table><thead><tr><th>#</th><th>الدكتور</th><th>KPI</th><th>عدد الفواتير</th><th>الصافي</th><th>متوسط الفاتورة</th><th>المتوسط اليومي</th></tr></thead><tbody>${doctorRows}</tbody></table><h3>مؤشرات جودة الأداء</h3><table><thead><tr><th>#</th><th>الدكتور</th><th>أصناف/فاتورة</th><th>الأصناف المختلفة</th><th>فواتير فوق 100</th><th>الثبات</th></tr></thead><tbody>${qualityRows}</tbody></table></section><section class="section page-break"><h2 class="section-title">أكثر 30 صنف مبيعًا</h2><table><thead><tr><th>#</th><th>الصنف</th><th>الكمية</th></tr></thead><tbody>${topRows}</tbody></table></section>${shortageRows?`<section class="section page-break"><h2 class="section-title">النواقص المقترحة</h2><table><thead><tr><th>#</th><th>الصنف</th><th>الرصيد</th><th>المعدل اليومي</th><th>المقترح</th></tr></thead><tbody>${shortageRows}</tbody></table></section>`:''}<section class="section page-break"><h2 class="section-title">مقارنة الفترة بالتفصيل</h2>${compare}</section><section class="section page-break"><h2 class="section-title">غيابات الموظفين</h2><p>${esc(state.reportBuilderAbsences||'—').replace(/\n/g,'<br>')}</p><h2 class="section-title">المشاكل</h2><p>${esc(state.reportBuilderProblems||'—').replace(/\n/g,'<br>')}</p><h2 class="section-title">الملاحظات</h2><p>${esc(state.reportBuilderNotes||'—').replace(/\n/g,'<br>')}</p><h2 class="section-title">الإنجازات</h2><p>${esc(state.reportBuilderAchievements||'—').replace(/\n/g,'<br>')}</p><h2 class="section-title">الخطة القادمة</h2><p>${esc(state.reportBuilderPlan||'—').replace(/\n/g,'<br>')}</p></section>`;
...
function resetItemCatalogModal(){
  const modal=showModal('حذف دليل الأصناف بالكامل',`<div class="catalog-reset-warning"><strong>هذه العملية تحذف دليل الأصناف الحالي بالكامل.</strong><p>تقارير حركة الأصناف القديمة لن تُحذف، لكن المطابقات المرتبطة بالدليل القديم ستُلغى. بعد الحذف يمكنك استيراد دليل أصناف جديد مباشرة.</p></div><div class="field"><label>للتأكيد اكتب: <b>حذف الدليل</b></label><input class="input" id="catalogResetConfirm" autocomplete="off" placeholder="حذف الدليل"></div>`,async()=>{
    ...
    try{
      ...
      state.items=[];state.itemCatalogTotal=0;state.itemCatalogSearch='';state.itemCatalogShowAll=false;invalidateItemSalesRates();
...
function supplierBranchTags(s){
  const names=(s?.branch_names||[]).filter(Boolean);
  if(!names.length){
    const selected=state.branches.find(b=>b.id===(state.supplierBranchId||''));
    return selected?`<span class="branch-chip">${esc(selected.name)}</span>`:'<span class="muted">—</span>';
  }
  return `<div class="supplier-branch-tags">${names.map(n=>`<span class="branch-chip">${esc(n)}</span>`).join('')}</div>`;
}
...
function renderSupplierRows(){
  ...
  const actions=s=>{const planBtn=can('manage_payment_plans')?`<button class="btn btn-soft btn-sm" data-supplier-plan="${s.id}">خطة سداد</button>`:'';return `<button class="btn btn-primary btn-sm" data-open="${s.id}">فتح</button><button class="btn btn-soft btn-sm" data-summary="${s.id}">كشف</button>${planBtn}${can('manage_suppliers')?`<button class="btn btn-ghost btn-sm" data-edit="${s.id}">تعديل</button><button class="btn btn-danger btn-sm" data-delete="${s.id}">حذف</button>`:''}`;};
  box.innerHTML=`<div class="table-wrap desktop-table"><table><thead><tr><th>المورد</th><th>التصنيفات</th><th>حالة الدين</th><th>القيمة</th><th>Aging</th><th>الهاتف</th><th>الفرع</th><th></th></tr></thead><tbody>${rows.map(s=>`<tr><td><strong>${esc(s.name)}</strong></td><td>${categoryTags(s.categories)}</td><td>${supplierDebtBadge(s.balance)}</td><td class="money supplier-balance-cell ${supplierDebtState(s.balance)}">${supplierDebtMoney(s.balance)}</td><td>${supplierAgingBadge(s.aging_days)}</td><td>${esc(s.phone||'-')}</td><td>${supplierBranchTags(s)}</td><td><div class="actions">${actions(s)}</div></td></tr>`).join('')}</tbody></table></div><div class="mobile-list">${rows.map(s=>`<div class="item-card"><div class="item-title supplier-item-title"><div><span>${esc(s.name)}</span>${categoryTags(s.categories)}</div><div class="supplier-mobile-balance"><small>${supplierDebtLabel(s.balance)}</small><strong class="money ${supplierDebtState(s.balance)}">${supplierDebtMoney(s.balance)}</strong></div></div><div class="item-meta"><div><span>حالة الدين</span>${supplierDebtBadge(s.balance)}</div><div><span>Aging · أقدم فاتورة مفتوحة</span>${supplierAgingBadge(s.aging_days)}</div><div><span>الهاتف</span>${esc(s.phone||'-')}</div><div><span>الفرع</span>${supplierBranchTags(s)}</div></div><div class="item-actions">${actions(s)}</div></div>`).join('')||'<div class="empty">لا توجد نتائج</div>'}</div>`;
...
function openSupplierPaymentPlanModal(supplierId){
  const supplier=state.supplierRows.find(s=>s.id===supplierId)||state.suppliers.find(s=>s.id===supplierId);
  if(!supplier){toast('تعذر تحديد المورد',true);return;}
  const branchSelect=document.getElementById('supplierBranchFilter');
  const branchId=state.supplierBranchId||branchSelect?.value||'';
  if(!branchId){toast('اختر الفرع أولًا قبل إضافة خطة سداد من صفحة الموردين',true);return;}
  const balance=Number(supplier.balance||0);
  if(balance>=0){toast(balance>0?'هذا المورد عليه دين لنا، وليس دين علينا يحتاج خطة سداد':'هذا المورد بدون دين يحتاج خطة سداد',true);return;}
  const branchName=branchSelect?.selectedOptions?.[0]?.textContent?.trim()||'الفرع المحدد';
  paymentPlanModal(null,{supplierId:supplier.id,branchId,plannedAmount:Math.abs(balance),sourceLabel:`${supplier.name} · ${branchName}`,onSaved:async()=>{await refreshSupplierRows();}});
}
...
function renderSupplierImportPreview(wrap,rows){
  ...
  box.innerHTML=`<div class="supplier-import-summary">تم استخراج <b>${rows.length}</b> صف. الموجب = دين لنا، السالب = دين علينا، الصفر = بدون دين. عدّل قبل التأكيد.</div><div class="supplier-import-table"><table><thead><tr><th>استيراد</th><th>اسم المورد</th><th>ر.م</th><th>الرصيد</th><th>الحالة</th><th>آخر سداد</th><th>آخر فاتورة</th><th></th></tr></thead><tbody>${rows.map((r,i)=>`<tr data-import-row><td><input type="checkbox" class="supplierImportInclude" checked></td><td><input class="input supplierImportName" value="${esc(r.name||'')}" required></td><td><input class="input supplierImportRef" value="${esc(r.reference_no||'')}"></td><td><input class="input supplierImportBalance" type="number" step="0.01" value="${Number(r.balance||0)}"></td><td class="supplierImportState">${supplierDebtBadge(Number(r.balance||0))}</td><td><input class="input supplierImportPaid" type="date" value="${esc(r.last_payment_date||'')}"></td><td><input class="input supplierImportInvoice" type="date" value="${esc(r.last_invoice_date||'')}"></td><td><button class="btn btn-danger btn-sm supplierImportDelete" type="button">حذف</button></td></tr>`).join('')}</tbody></table></div><div class="hint" id="supplierImportCount">${rows.length} صف جاهز للاستيراد</div>`;
  box.querySelectorAll('.supplierImportBalance').forEach(input=>input.addEventListener('input',()=>updateSupplierImportRowState(input)));
  box.querySelectorAll('.supplierImportInclude').forEach(input=>input.addEventListener('change',updateSupplierImportCount));
  box.querySelectorAll('.supplierImportName').forEach(input=>input.addEventListener('input',updateSupplierImportCount));
  box.querySelectorAll('.supplierImportDelete').forEach(btn=>btn.addEventListener('click',()=>{btn.closest('[data-import-row]')?.remove();updateSupplierImportCount();}));
}
...
function paymentPlanSupplierCategoryIds(supplierId){
  const supplier=state.suppliers.find(s=>s.id===supplierId);
  return new Set((supplier?.categories||[]).map(c=>c.id));
}
function paymentPlanSupplierCategoriesText(supplierId){
  const supplier=state.suppliers.find(s=>s.id===supplierId);
  return (supplier?.categories||[]).map(c=>c.name).filter(Boolean).join('، ')||'بدون تصنيف';
}
function paymentPlanCategoryOptions(){
  return `<option value="">كل التصنيفات</option>${state.categories.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}`;
}
async function paymentPlansView(main){
  ...
  if(!state.paymentPlanWeek)state.paymentPlanWeek=isoWeekValue();
  main.innerHTML=`<div class="page-head"><div><h2>خطة السداد الأسبوعية</h2><div class="muted">نظم مواعيد السداد أسبوعيًا واعرف المطلوب قبل موعده</div></div><div class="page-head-actions"><button class="btn btn-soft" id="paymentPlanPdf">تصدير PDF</button>${can('manage_payment_plans')?'<button class="btn btn-primary" id="addPaymentPlan">+ موعد سداد</button>':''}</div></div>
  <div id="paymentPlanSummary"></div>
  <section class="panel payment-plan-filter-panel"><div class="toolbar payment-plan-toolbar"><input class="input" id="paymentPlanSearch" placeholder="بحث باسم المورد..." value="${esc(state.paymentPlanSearch||'')}"><select class="select" id="paymentPlanBranch">${branchOptions(true,false)}</select><select class="select" id="paymentPlanCategory">${paymentPlanCategoryOptions()}</select><select class="select" id="paymentPlanStatus"><option value="open">المواعيد المفتوحة</option><option value="overdue">المتأخرة</option><option value="due_today">مستحقة اليوم</option><option value="upcoming">القادمة</option><option value="postponed">المؤجلة</option><option value="completed">تم السداد</option><option value="cancelled">الملغاة</option><option value="all">الكل</option></select><input class="input" type="week" id="paymentPlanWeek" title="الأسبوع"><button class="btn btn-soft btn-sm" id="paymentPlanAllWeeks" type="button">كل الأسابيع</button></div><div class="hint">اختار الأسبوع والتصنيف لعرض خطة السداد المطلوبة، أو كل الأسابيع لعرض كل المواعيد.</div></section><div id="paymentPlanRows"></div>`;
  const branch=document.getElementById('paymentPlanBranch'),category=document.getElementById('paymentPlanCategory'),status=document.getElementById('paymentPlanStatus'),search=document.getElementById('paymentPlanSearch'),week=document.getElementById('paymentPlanWeek');branch.value=state.paymentPlanBranchId||'';category.value=state.paymentPlanCategoryId||'';status.value=state.paymentPlanStatus||'open';week.value=state.paymentPlanWeek||'';
  if(can('manage_payment_plans'))document.getElementById('addPaymentPlan').onclick=()=>paymentPlanModal(null,{plannedDate:isoToday()});
  document.getElementById('paymentPlanPdf').onclick=exportPaymentPlanPdf;document.getElementById('paymentPlanAllWeeks').onclick=()=>{state.paymentPlanWeek='';week.value='';renderPaymentPlanRows();};
  const refresh=()=>{state.paymentPlanBranchId=branch.value;state.paymentPlanCategoryId=category.value;state.paymentPlanStatus=status.value;state.paymentPlanSearch=search.value;state.paymentPlanWeek=week.value;renderPaymentPlanRows();};branch.oninput=refresh;category.oninput=refresh;status.oninput=refresh;search.oninput=refresh;week.oninput=refresh;renderPaymentPlanRows();
}
function paymentPlanWeekSummaryMarkup(rows){
  const range=state.paymentPlanWeek?isoWeekRange(state.paymentPlanWeek):null;
  const planned=rows.filter(p=>['planned','postponed'].includes(p.status)).reduce((a,p)=>a+Number(p.planned_amount||0),0);
  const completed=rows.filter(p=>p.status==='completed').reduce((a,p)=>a+Number(p.actual_amount||p.planned_amount||0),0);
  const overdue=rows.filter(p=>p.display_status==='overdue').reduce((a,p)=>a+Number(p.planned_amount||0),0);
  return `<div class="cards weekly-plan-summary"><div class="stat"><div class="label">${range?'خطة الأسبوع':'خطة السداد'}</div><div class="value">${esc(range?range.label:'كل الأسابيع')}</div></div><div class="stat"><div class="label">إجمالي المخطط</div><div class="value">${money(planned)}</div></div><div class="stat"><div class="label">تم سداده</div><div class="value">${money(completed)}</div></div><div class="stat"><div class="label">متأخر</div><div class="value">${money(overdue)}</div></div></div>`;
}
function visiblePaymentPlanRows(){
  const q=(state.paymentPlanSearch||'').trim().toLowerCase(),filter=state.paymentPlanStatus||'open',categoryId=state.paymentPlanCategoryId||'';
  const scope=state.paymentPlans.filter(p=>(!state.paymentPlanBranchId||p.branch_id===state.paymentPlanBranchId)&&(!categoryId||paymentPlanSupplierCategoryIds(p.supplier_id).has(categoryId))&&(!state.paymentPlanWeek||dateInIsoWeek(p.planned_date,state.paymentPlanWeek))&&(!q||String(p.supplier_name||'').toLowerCase().includes(q)));
  ...
  const priority={overdue:0,due_today:1,postponed:2,planned:3,completed:4,cancelled:5};return rows.sort((a,b)=>(priority[a.display_status]??9)-(priority[b.display_status]??9)||String(a.planned_date).localeCompare(String(b.planned_date)));
}
function exportPaymentPlanPdf(){
  const rows=visiblePaymentPlanRows(),range=state.paymentPlanWeek?isoWeekRange(state.paymentPlanWeek):null,branch=state.paymentPlanBranchId?(state.branches.find(b=>b.id===state.paymentPlanBranchId)?.name||'فرع محدد'):'كل الفروع',category=state.paymentPlanCategoryId?(state.categories.find(c=>c.id===state.paymentPlanCategoryId)?.name||'تصنيف محدد'):'كل التصنيفات';
  const statusMap={open:'المواعيد المفتوحة',overdue:'المتأخرة',due_today:'مستحقة اليوم',upcoming:'القادمة',postponed:'المؤجلة',completed:'تم السداد',cancelled:'الملغاة',all:'الكل'};
  const planned=rows.filter(p=>['planned','postponed'].includes(p.status)).reduce((a,p)=>a+Number(p.planned_amount||0),0),completed=rows.filter(p=>p.status==='completed').reduce((a,p)=>a+Number(p.actual_amount||p.planned_amount||0),0),overdue=rows.filter(p=>p.display_status==='overdue').reduce((a,p)=>a+Number(p.planned_amount||0),0);
  const popup=window.open('','_blank');if(!popup){toast('المتصفح منع نافذة التصدير. اسمح بالنوافذ المنبثقة.',true);return;}
  const rowHtml=rows.map((p,i)=>`<tr><td>${i+1}</td><td class="name">${esc(p.supplier_name)}</td><td>${esc(paymentPlanSupplierCategoriesText(p.supplier_id))}</td><td>${esc(paymentPlanSupplierCategoriesText(p.supplier_id))}</td><td>${esc(p.branch_name)}</td><td class="money">${money(p.planned_amount)}</td><td class="money">${p.status==='completed'&&p.actual_amount!=null?money(p.actual_amount):'—'}</td><td>${esc(p.planned_date)}</td><td>${esc(PAYMENT_PLAN_STATUS_LABELS[p.display_status||p.status]||p.status||'—')}</td><td>${esc(p.notes||p.last_postpone_reason||'')}</td></tr>`).join('')||'<tr><td colspan="9">لا توجد مواعيد مطابقة</td></tr>';
  popup.document.write(`<!doctype html><html lang="ar" dir="rtl"><head><meta charset="utf-8"><title>خطة السداد الأسبوعية</title><style>@page{size:A4 landscape;margin:9mm}*{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}body{font-family:Tahoma,Arial,sans-serif;color:#173b36;margin:0;direction:rtl}.head{border-bottom:3px solid #0f6259;padding-bottom:8px;margin-bottom:10px;display:flex;justify-content:space-between;gap:20px}.head h1{margin:0;color:#0f6259;font-size:21px}.meta{font-size:10px;color:#60736e;line-height:1.8}.cards{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin:10px 0}.card{border:1px solid #d7e2df;border-radius:8px;padding:8px;background:#f8fbfa}.card span{display:block;color:#667872;font-size:9px}.card strong{font-size:14px;color:#0f6259}.filters{background:#eef7f4;border:1px solid #d7e2df;border-radius:8px;padding:7px;margin-bottom:9px;font-size:10px}table{width:100%;border-collapse:collapse;font-size:9px;table-layout:fixed}th,td{border:1px solid #dbe5e2;padding:5px;text-align:center;vertical-align:middle;word-break:break-word}th{background:#0f6259;color:#fff}.name{text-align:right;font-weight:700}.money{direction:ltr;white-space:nowrap;font-variant-numeric:tabular-nums}tbody tr:nth-child(even){background:#f7faf9}.no-print{text-align:center;margin-top:10px}@media print{.no-print{display:none}}</style></head><body><div class="head"><div><h1>خطة السداد الأسبوعية</h1><div class="meta">الأسبوع: <strong>${esc(range?range.label:'كل الأسابيع')}</strong><br>الفرع: <strong>${esc(branch)}</strong> · التصنيف: <strong>${esc(category)}</strong> · الحالة: <strong>${esc(statusMap[state.paymentPlanStatus]||'الكل')}</strong></div></div><div class="meta">Abdo Debts<br>تاريخ التصدير: ${esc(doctorPdfGeneratedAt())}</div></div><div class="cards"><div class="card"><span>إجمالي المخطط المفتوح</span><strong>${money(planned)}</strong></div><div class="card"><span>تم سداده</span><strong>${money(completed)}</strong></div><div class="card"><span>المتبقي المفتوح</span><strong>${money(Math.max(0,planned-completed))}</strong></div><div class="card"><span>متأخر</span><strong>${money(overdue)}</strong></div></div><div class="filters">عدد المواعيد: <strong>${rows.length.toLocaleString('en-US')}</strong>${state.paymentPlanSearch?` · البحث: <strong>${esc(state.paymentPlanSearch)}</strong>`:''}</div><table><thead><tr><th>#</th><th>المورد</th><th>التصنيف</th><th>الفرع</th><th>المبلغ المخطط</th><th>المدفوع الفعلي</th><th>موعد السداد</th><th>الحالة</th><th>ملاحظات</th></tr></thead><tbody>${rowHtml}</tbody></table><div class="no-print"><button onclick="print()" style="padding:8px 18px;border:0;border-radius:8px;background:#0f6259;color:white;font:inherit">حفظ كـ PDF / طباعة</button></div></body></html>`);popup.document.close();toast('تم تجهيز PDF خطة السداد.');setTimeout(()=>{try{popup.focus();popup.print();}catch{}},450);
}
function renderPaymentPlanRows(){
  ...
  const bid=state.paymentPlanBranchId||'',categoryId=state.paymentPlanCategoryId||'',q=(state.paymentPlanSearch||'').trim().toLowerCase(),filter=state.paymentPlanStatus||'open';
  const scope=state.paymentPlans.filter(p=>(!bid||p.branch_id===bid)&&(!categoryId||paymentPlanSupplierCategoryIds(p.supplier_id).has(categoryId))&&(!state.paymentPlanWeek||dateInIsoWeek(p.planned_date,state.paymentPlanWeek))&&(!q||(p.supplier_name||'').toLowerCase().includes(q)));
  ...
  document.getElementById('paymentPlanSummary').innerHTML=paymentPlanWeekSummaryMarkup(rows);
  ...
  box.innerHTML=`<div class="table-wrap desktop-table"><table><thead><tr><th>المورد</th><th>التصنيف</th><th>الفرع</th><th>المبلغ المخطط</th><th>الدين الحالي</th><th>موعد السداد</th><th>الحالة</th><th>ملاحظات</th><th></th></tr></thead><tbody>${rows.map(p=>`<tr><td><strong>${esc(p.supplier_name)}</strong>${Number(p.postpone_count)>0?`<div class="muted">تأجل ${Number(p.postpone_count)} مرة</div>`:''}</td><td>${esc(paymentPlanSupplierCategoriesText(p.supplier_id))}</td><td>${esc(p.branch_name)}</td><td class="money">${money(p.planned_amount)}${completedInfo(p)}</td><td class="money">${money(p.current_balance)}</td><td><strong>${esc(p.planned_date)}</strong></td><td>${paymentPlanStatusBadge(p)}</td><td>${esc(p.notes||p.last_postpone_reason||'-')}</td><td><div class="actions">${actions(p)}</div></td></tr>`).join('')}</tbody></table></div><div class="mobile-list">${rows.map(p=>`<div class="item-card payment-plan-card"><div class="item-title"><span>${esc(p.supplier_name)}</span>${paymentPlanStatusBadge(p)}</div><div class="muted" style="margin-top:5px">${esc(p.branch_name)} · ${esc(paymentPlanSupplierCategoriesText(p.supplier_id))}</div><div class="item-meta"><div><span>المبلغ المخطط</span><b>${money(p.planned_amount)}</b>${completedInfo(p)}</div><div><span>موعد السداد</span><b>${esc(p.planned_date)}</b></div><div><span>الدين الحالي</span><b>${money(p.current_balance)}</b></div><div><span>مرات التأجيل</span>${Number(p.postpone_count||0)}</div></div>${p.notes?`<div class="plan-note">${esc(p.notes)}</div>`:''}<div class="item-actions">${actions(p)}</div></div>`).join('')||'<div class="empty">لا توجد مواعيد مطابقة</div>'}</div>`;
...
function paymentPlanModal(plan=null,opts={}){
  const lockedSupplier=opts.supplierId||'',lockedBranch=opts.branchId||'';
  const initialSupplier=plan?.supplier_id||lockedSupplier||'';
  const initialAmount=plan?.planned_amount??(opts.plannedAmount?Number(opts.plannedAmount).toFixed(2):'');
  const sourceNote=opts.sourceLabel?`<div class="hint full">تم فتح خطة السداد من صفحة الموردين: ${esc(opts.sourceLabel)}</div>`:'';
  showModal(`${plan?'تعديل':'إضافة'} موعد سداد`,`<form id="paymentPlanForm" class="form-grid">${sourceNote}<div class="field"><label>المورد *</label>${paymentPlanSupplierComboboxMarkup(initialSupplier)}</div><div class="field"><label>الفرع *</label><select class="select" name="branch_id" required><option value="">اختر الفرع</option>${branchOptions(false)}</select></div><div class="field"><label>المبلغ المخطط *</label><input class="input" type="number" name="planned_amount" min="0.01" step="0.01" required value="${initialAmount}"></div><div class="field"><label>موعد السداد *</label><input class="input" type="date" name="planned_date" required value="${plan?.planned_date||opts.plannedDate||isoToday()}"></div><div class="field full"><label>ملاحظات</label><textarea class="textarea" name="notes" placeholder="مثال: دفعة حسب الاتفاق مع الشركة">${esc(plan?.notes||'')}</textarea><div class="hint">النظام يمنع أن يتجاوز مجموع المبالغ المخططة الدين المتبقي للمورد في الفرع.</div></div></form>`,async()=>{const f=document.getElementById('paymentPlanForm');if(!f.reportValidity())return false;const fd=new FormData(f);const payload={supplier_id:lockedSupplier||fd.get('supplier_id'),branch_id:lockedBranch||fd.get('branch_id'),planned_amount:Number(fd.get('planned_amount')),planned_date:fd.get('planned_date'),notes:fd.get('notes')||null};const saved=await api(plan?`/api/payment-plans/${plan.id}`:'/api/payment-plans',{method:plan?'PUT':'POST',body:JSON.stringify(payload)});toast('تم حفظ موعد السداد');if(opts.onSaved)await opts.onSaved(saved);else await paymentPlansView(document.getElementById('main'));return true;});
  const f=document.getElementById('paymentPlanForm');f.elements.branch_id.value=plan?.branch_id||lockedBranch||'';if(lockedBranch)f.elements.branch_id.disabled=true;const combo=wirePaymentPlanSupplierCombobox();if(lockedSupplier){const supplierInput=document.getElementById('planSupplierSearch'),supplierToggle=document.getElementById('planSupplierToggle');if(supplierInput)supplierInput.disabled=true;if(supplierToggle)supplierToggle.disabled=true;combo?.setSupplier(lockedSupplier);} 
}
...
function userModal(u=null){
 ...
 <div class="field"><label>كلمة المرور ${u?'<span class="muted">اتركها فارغة بدون تغيير</span>':'*'}</label><input class="input" type="password" name="password" ${u?'':'required'} minlength="10" autocomplete="new-password"><div class="hint">10 أحرف على الأقل</div></div><div class="field"><label>الدور</label><select class="select" name="role"><option value="finance">مالي</option><option value="viewer">مشاهدة فقط</option>${state.profile?.role==='admin'?'<option value="admin">مدير</option>':''}</select></div>
