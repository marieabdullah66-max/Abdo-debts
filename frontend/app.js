...
function exportReportBuilderPdf(){
 ...
 const compare=state.periodComparePrevious&&state.periodCompareCurrent?`<div class="note-box">مقارنة الفترة: ${esc(periodComparePeriodLabel(state.periodComparePrevious))} مقابل ${esc(periodComparePeriodLabel(state.periodCompareCurrent))}</div><table><thead><tr><th>المؤشر</th><th>الفترة السابقة</th><th>الفترة الحالية</th><th>التغير</th></tr></thead><tbody>${['net_sales','invoice_count','average_invoice','daily_average','kpi_average'].map(k=>{const a=k==='kpi_average'?state.periodComparePrevious.totals?.kpi_average:state.periodComparePrevious.totals?.[k],b=k==='kpi_average'?state.periodCompareCurrent.totals?.kpi_average:state.periodCompareCurrent.totals?.[k];return `<tr><td>${k}</td><td>${a??'—'}</td><td>${b??'—'}</td><td>${a!=null&&b!=null?((Number(b)-Number(a)).toFixed(2)):'—'}</td></tr>`}).join('')}</tbody></table>`:'<div class="note-box">لا توجد مقارنة فترة محملة.</div>';
